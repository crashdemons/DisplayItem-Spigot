 ...
 * @author crash
 */
public abstract class DeferredEventSpamPreventer {
    /**
     ...
     */
    protected final DeferredEventSpamRecord[] records; // = new EventSpamRecord[RECORDS];
    private volatile int next = 0;
    
    public DeferredEventSpamPreventer(int numRecords){
        recordCount=numRecords;
        records=new DeferredEventSpamRecord[recordCount];
    }
     ...
     */
    protected synchronized void addRecord(DeferredEventSpamRecord record){
        records[next] = record;
     ...
     * @see #addRecord(com.github.crashdemons.playerheads.antispam.EventSpamRecord) 
     */
    public abstract SpamResult recordEventParams(DeferredChatEventParameters params);
}
